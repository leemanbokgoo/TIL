...
### Fetch join 의 단점
- 연관관계 설정해놓은 FetchType을 사용할 수 없다. Fetch join을 사용하게되면 데이터 호출 시점에 모든 연관관계의 데이터를 가져오기때문에 FetchType을 Lazy로 해놓는 것이 무의미하다.
- 쿼리를 한꺼번에 가져오기때문에 JPA가 제공하는 Paging API 사용 불가능(Pageable 사용 불가 )
- 1:N 관계가 두개 이상인 경우 사용불가
...
- Fetch Join과 EntityGraph는 JPQL을 사용하여 JOIN문을 호출한다는 공통점이 있다. 또한, 공통적으로 카테시안 곱(Cartesian Product)이 발생하여 Owner의 수만큼 Cat이 중복 데이터가 존재할 수 있다. 그러므로 중복된 데이터가 컬렉션(리스트 , N에 해당하는 하위 엔티티 객체 )에 존재하지 않도록 주의해야 한다.
...
- 즉시로딩인 경우 엔티티를 조회하는 시점에 연관관계 엔티티를 같이 조회한다. @BatchSize가 있으므로 연관관계 엔티티의 row 갯수만큼 추가 SQL을 날리지 않고, 조회한 엔티티의 id들을 모아서 SQL IN 절을 날린다. size는 IN절에 올수있는 최대 인자 개수를 말한다. 만약 batch size가 5일때 연관관계 엔티티의 개수가 10개라면 위의 IN절이 2번 실행될것이다. 그리고 만약 지연 로딩이라면 지연 로딩된 엔티티 최초 사용시점에 5건을 미리 로딩해두고, 6번째 엔티티 사용 시점에 다음 SQL을 추가로 실행한다.
