...
- 웹 소켓이란 두 프로그램 간의 메시지 교환을 위한 통신 방법 중 하나로 HTML5에 등장 실시간 웹 애플리케이션을 위해 설계된 통신 프로토콜이며, TCP(Transmission Control Protocol)를 기반으로 한다. 웹소켓 프로토콜은 HTTP와 다른 통신 프로토콜로 웹서버와 웹 브라우저가 실시간 메세지를 교환하는데 사용되며 웹소켓 연결을 맺기위한 첫번째 핸드 쉐이크를 주고받은 이후 지속적으로 연결이 유지되는 것이 특징이며 매번 메세지 전송 시에 새롭게 연결을 맺을 필요가 없어 빠르고 효율적이다.
...
- 지속적으로 연결 상태를 유지하므로 보안에 취약할 수 있다.
- 프록시나 방화벽이 웹소켓 트래픽을 차단할 수 있다.
...
### 질문
####  WebSocket과 SSE는 모두 실시간 통신을 위해 사용되는데, 이 둘의 주요 차이점은 무엇이며 각각 어떤 상황에서 적합한가요?
- WebSocket은 클라이언트와 서버가 서로 데이터를 주고받을 수 있는 양방향 통신을 지원하는 반면, SSE는 서버가 클라이언트에게만 데이터를 일방향으로 푸시하는 방식입니다. WebSocket은 초기에는 HTTP로 연결을 시작하지만 이후 업그레이드를 통해 별도의 프로토콜로 전환되며, 지속적인 양방향 소켓 연결이 유지됩니다. 반면 SSE는 HTTP 연결을 유지하면서 텍스트 기반의 이벤트 스트림을 통해 서버가 데이터를 전송합니다. 따라서 채팅, 게임, 협업 앱 등 양방향 소통이 필요한 경우에는 WebSocket이 적합하고, 주가 알림, 뉴스 속보, 서버 상태 모니터링 등 서버에서 클라이언트로만 정보 전달이 필요한 경우에는 SSE가 더 간단하고 효율적입니다.

### WebSocket과 SSE 중 브라우저 및 네트워크 환경에서의 호환성과 제약 측면에서 고려해야 할 점은 무엇인가요?
- WebSocket은 모던 브라우저 대부분에서 지원되지만, 프록시나 방화벽이 WebSocket 연결을 차단하거나 비정상적으로 처리할 수 있어 네트워크 환경에 따라 제한이 있을 수 있습니다. 또한 WebSocket은 헤더 커스터마이징이 어렵고, 리커넥션 로직을 직접 구현해야 하는 번거로움이 있습니다. 반면 SSE는 HTTP 기반이므로 방화벽 우회에 유리하고, 자동 재연결, 이벤트 ID를 통한 메시지 재전송 지원 등 안정성을 제공합니다. 그러나 SSE는 기본적으로 HTTP/1 기반이므로 연결 수 제한 문제가 있고, 브라우저에서만 사용 가능하며 양방향 통신이 불가능하다는 단점도 있습니다. 이러한 점들을 고려해 프로젝트 환경에 맞는 방식을 선택해야 합니다.
