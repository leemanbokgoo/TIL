...
### 질문
#### DNS의 역할이 무엇인가요?
- DNS(Domain Name System)는 웹사이트의 도메인 이름을 IP 주소로 변환하여 사용자가 사람이 이해하기 쉬운 도메인 이름을 통해 사이트에 접근할 수 있도록 도와주는 시스템입니다. 이는 인터넷의 전화번호부와 같은 역할을 하며, 사용자가 URL을 입력할 때마다 자동으로 도메인과 IP 주소를 매핑해 줍니다.

#### TCP 연결을 할 때 3-way handshake가 필요한 이유는 무엇인가요?
- 3-way handshake는 클라이언트와 서버 간의 데이터 전송을 위한 안정적인 연결을 보장하기 위해 필요합니다. 클라이언트와 서버가 서로의 존재를 확인하고 연결을 승인하는 과정을 통해 데이터 손실을 방지하고, 데이터가 안정적으로 전송될 준비가 되었음을 보장합니다.
