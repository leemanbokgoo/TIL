...
### 멀티 스레드인 memcached가 왜 더 빠르지않을까?
- Redis는 네트워크 I/O → 데이터 조회/수정 → 응답 전송 과정 전체를 아주 빠르게, 메모리 기반으로 끝낸다. 그리고 하나의 명령을 엄청 빠르게 끝내고 다음 명령을 처리하기 때문에, 실제로는 엄청난 처리량(throughput)을 유지할 수 있다. 심지어 Redis는 싱글 스레드지만, 내부 구현이 CPU 캐시 친화적이고, context switching 비용도 없다. 싱글 스레드지만 병렬 못 하는 대신, 워낙 빨리 처리해서 사실상 병렬처럼 빠르다.
-  Memcached는 멀티스레드지만, 작업 자체가 단순해서 Redis 대비 이점이 제한적이다.Memcached는 단순 key-value 저장만 지원한다. 멀티스레드라서 요청은 동시에 여러 개 받을 수 있는데 실제 작업(메모리 읽기/쓰기)이 엄청 무겁지 않기 때문에, 병렬성 이득이 크진 않다. 멀티 스레드라도, 단순 작업에서는 스레드 간 락(lock)이나 컨텍스트 스위칭 비용이 더 커질 수도 있다.
-  Redis가 지원하는 다양한 명령어 최적화 덕분에 오히려 더 빠를 때도 있다. Redis는 단순 key-value뿐 아니라 리스트, 셋, 해시 같은 복잡한 자료구조를 메모리에서 다룰 수 있다. 이런 고급 명령들을 하나의 요청으로 빠르게 처리할 수 있어서, "요청 1번당 처리되는 데이터량"이 커진다. 반면 Memcached는 "그냥 key-value"만 다루기 때문에, 복잡한 처리를 애플리케이션 쪽에서 따로 해야 한다.
- 즉, Redis는 싱글스레드 기반이지만, 네트워크 I/O부터 데이터 처리까지 메모리 기반으로 매우 빠르게 처리하도록 최적화되어 있어, 실제로는 높은 처리량을 유지할 수 있다. 반면 Memcached는 멀티스레드를 사용해 여러 요청을 동시에 처리할 수 있지만, 단순한 key-value 저장만 지원하고 작업 자체가 가벼워 병렬성의 이점이 크지 않다. 또한 멀티스레드는 락과 컨텍스트 스위칭 비용이 추가로 발생할 수 있어, 항상 성능이 더 좋은 것은 아니다. Redis는 다양한 자료구조와 고급 명령어를 지원해 하나의 요청으로 많은 작업을 처리할 수 있기 때문에, 오히려 Memcached보다 빠르거나 비슷한 성능을 보이는 경우가 많다. 따라서 단순히 싱글스레드냐 멀티스레드냐만으로 성능을 비교할 수 없고, 작업의 특성과 시스템 최적화 정도를 함께 고려해야 한다.

![Image](https://github.com/user-attachments/assets/8f04298d-3df6-44c7-9aa8-1a87ba0175b7)


## Redis와 세션
-  세션 데이터는 단순한 문자열 하나가 아니다 보통의 세션데이터는 아래와 같은 데이터 구조를 가진다.(메모리에는 내부적으로 JSON 대신 Redis Hash 같은 구조로 저장할 수도 있다) 
    - Key: 세션 ID
    - Value: 사용자 정보 같은 여러 속성들(필드)로 구성된 JSON 같은 구조
![Image](https://github.com/user-attachments/assets/a3257631-a1fb-439b-b733-ddb67af967ae)
- 사용자가 닉네임을 "홍길동" ➔ "홍삼길동"으로 바꿨다고 할때 Redis에서는 해시(Hash) 타입 덕분에 nickname 필드만 수정할 수 있다. 반면, Memcached는 전체 세션 데이터를 통째로 꺼내서 수정 후 다시 전체를 저장해야 해야한다. 고로 낭비가 많고 느리다.
- 세션을 갱신할때 Redis에서는 TTL 갱신이 간단하다. Memcached도 TTL 갱신은 되지만, 이 경우도 전체 객체를 다시 넣는 식이기 때문에 효율이 떨어질 수 있다. 기본적으로 저장할 때 TTL을 지정할 수는 있지만 TTL만 따로 갱신하는 기능이 없다. 
- 세션을 갱신하는 상황
    - 사용자가 로그인한 지 시간이 지났을 때, 세션 만료(TTL)가 다 돼가니까 TTL(만료시간)을 다시 연장하는 것
    - 또는 사용자가 활동 중이면, 세션 만료 시간을 다시 밀어주는 것 ("활동이 있으니 세션을 연장합니다")
- Memcached는 이런 작업이 매번 전체 객체를 수정, 저장하는 식이라 비효율적이다. 
- 즉 세션 데이터는 단순한 문자열이 아니라 여러 속성으로 구성된 구조화된 데이터이다. Redis는 해시 타입을 사용해 세션 데이터의 일부 속성만 빠르게 수정할 수 있고, TTL 관리나 세션 갱신도 유연하게 지원하기 때문에 복잡한 세션 관리에 유리하다. 반면, Memcached는 value 전체를 꺼내고 다시 저장해야 하기 때문에 세션의 일부분만 수정하는 작업이 비효율적이다.
- 특히 복잡한 세션 속성 업데이트, 유저 활동에 따른 세션 연장 같은 현실적인 세션 관리 요구사항을 생각했을 때, Redis가 세션 저장소로 더 적합하다.


### 질문 
#### 세션저장소로서 Redis와 Memcached를 비교해보시오.
- 단순한 세션 캐시 용도로만 보면 Memcached도 충분히 사용할 수 있지만, 실무에서는 Redis를 세션 저장소로 선택하는 경우가 많습니다.그 이유는 Redis가 키마다 TTL을 정교하게 관리할 수 있고, 세션 데이터를 리스트나 해시 같은 복합 구조로 다룰 수 있어서 세션 데이터 일부만 수정하는 것도 효율적으로 처리할 수 있기 때문입니다. 또한 Redis는 장애 복구를 위한 데이터 지속성 기능(RDB, AOF)을 제공하고, Sentinel이나 Cluster를 통해 고가용성과 수평 확장도 지원하기 때문에, 대규모 서비스에서 안정성과 운영 편의성을 고려했을 때 훨씬 유리합니다. 
- 반면 Memcached는 메모리 기반으로만 동작해서 서버 장애 시 세션 데이터가 모두 사라질 수 있고, TTL 관리나 복잡한 데이터 조작이 어렵기 때문에, 단순 캐시 성능은 뛰어나지만 세션 저장소로는 Redis에 비해 한계가 있습니다. 따라서, 단순성과 속도만 필요한 경우에는 Memcached도 고려할 수 있지만, 세션 안정성, 데이터 조작 유연성, 장애 대응까지 고려하면 Redis를 세션 저장소로 선택하는 것이 좋다. 또한 스프링 부트에서 Redis 통합 지원이 잘 되어 있어서, 개발 및 운영 편의성이 좋다.
...
- 클러스터 : 단일 노드로 운영할 경우 메모리 용량이나 처리 성능에 물리적인 한계가 있기 때문에, 더 큰 트래픽이나 데이터 양을 처리하려면 Redis 클러스터를 구성하게 됩니다. 클러스터를 통해 데이터를 여러 노드에 분산 저장(샤딩)하고, 여러 서버가 동시에 트래픽을 처리할 수 있도록 수평 확장할 수 있습니다. 
