...
- Java Stream을 사용하기 전에는 데이터를 컬렉션(Collection)으로 처리했는데 코딩이 길고 가독성이 불편하여 작업할 때 애로 사항이 많다. 그렇다면 기존 Java의 컬렉션 처리 방식에 비해 J
- 가독성이 좋아진다.
...
- 데이터를 병렬 처리 지원은, 멀티 스레드를 이용하기 때문에 데이터를 효율적으로 처리할 수 있는데 이는 대용량 데이터를 효율적으로 처리하기 위해 필수적인 기능이며, Java Stream API는 간단하게 parallel() 또는 parallelStream()이라는 연산을 추가하는 것만으로 병렬처리가 가능하다.
